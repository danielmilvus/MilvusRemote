...
if [[ $MYSQL_RESULT != "" ]]; then
    echo -e "${RED}It appears there is already a MySQL datsudo abase (${guacDb}) on ${mysqlHost}${NC}" 1>&2
...
if [ "${installRepeater}" = true]; then
	...
	cd uvncrep015-ws
	echo -e "${BLUE}Running Make on Repeater.${NC}"
	make &>> ${LOG}
	if [ $? -ne 0 ]; then
		echo -e "${RED}Failed. See ${LOG}${NC}" 1>&2
		exit 1
	...
	echo -e "${BLUE}Creating Repeater user...${NC}"
	useradd uvncrep
